...
#Launch oLIB
run_args = '-I %s -r %s -i %s -b %s -l %s --channel-types=%s --channel-names=%s --state-channels=%s --start=%s --stride=%s --overlap=%s --wait=%s --max-wait=%s --t-shift-start=%s --t-shift-stop=%s --t-shift-num=%s --dt-signal-kde-coords=%s --dt-signal-kde-values=%s --dt-noise-kde-coords=%s --dt-noise-kde-values=%s --FAR-thresh=%s --background-dic=%s --background-livetime=%s --oLIB-signal-kde-coords=%s --oLIB-signal-kde-values=%s --oLIB-noise-kde-coords=%s --oLIB-noise-kde-values=%s --bitmask=%s --inj-runmode=%s'%(ifos, rundir, infodir, bindir, lib_label, channel_types, channel_names, state_channels, actual_start, stride, overlap, wait, max_wait, t_shift_start, t_shift_stop, t_shift_num, dt_signal_kde_coords, dt_signal_kde_values, dt_noise_kde_coords, dt_noise_kde_values, FAR_thresh, back_dic_path, back_livetime, oLIB_signal_kde_coords, oLIB_signal_kde_values, oLIB_noise_kde_coords, oLIB_noise_kde_values, bitmask, inj_runmode)
if gdb_flag:
	run_args += " --gdb"
if LIB_flag:
	run_args += " --LIB"
