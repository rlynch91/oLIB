...
if __name__=='__main__':
	...
	parser.add_option("","--clustered", default=None, type="string", help="Comma separated list of paths to files containing clustered trigs for each ifo")
	...
	clustered = opts.clustered.split(",")
...
#		print "Clustered trigs for %s"%ifo
	...
	#load in clustered files
	if not os.path.exists("%s/clustered/"%ppdir):
		os.makedirs("%s/clustered/"%ppdir)
	os.system('rm %s/clustered/*'%ppdir)
	clust_files = {}
	for i,ifo in enumerate(ifos):
		os.system('cp %s %s/clustered/clustered_%s.txt'%(clustered[i],ppdir,ifo))		
		clust_files[ifo] = '%s/clustered/clustered_%s.txt'%(ppdir,ifo)
	...
	seg_files = {}
	for i,ifo in enumerate(ifos):
		os.system('cp %s %s/live_segs/%s.seg'%(segs[i],ppdir,ifo))
		segs[i] = '%s/live_segs/%s.seg'%(ppdir,ifo)
